...
Fat Fat::parse(std::span<const u8> data)
{
	...
	{
		fat.m_entries[i].start = readU32(data, i * FatEntry::SIZE);
		fat.m_entries[i].end   = readU32(data, i * FatEntry::SIZE + 4);
	}
	return fat;
}
...
std::vector<u8> Fat::serialize() const
{
	...
	{
		writeU32(span, i * FatEntry::SIZE, m_entries[i].start);
		writeU32(span, i * FatEntry::SIZE + 4, m_entries[i].end);
	}
	return out;
}
